...
class Device extends Backend
{
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\hj212\Device;
    }
    ...
    {
        $ids = $this->request->param('deviceId',0);
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                //检查设备号是否已经注册
                $device = $this->model->where(['device_code'=>$params['device_code']])->find();
                if($device){
                    $this->error("设备已存在，请勿重复添加");
                }
                $result = false;
                Db::startTrans();
                try {
